    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 text-white p-5 rounded-2xl shadow-lg border border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center space-x-3">
          <div className="h-10 w-10 rounded-xl bg-blue-500/20 text-blue-400 border border-blue-500/30 flex items-center justify-center font-bold">
      ...
      <form onSubmit={submeterForm} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-5">
            ...
            <div className="flex items-center space-x-2">
              ...
              {loggedUser.role === "ADMIN" && (
                <button
                  ...
                  className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold cursor-pointer transition-all flex items-center space-x-1"
                >
                  <i className="fa-solid fa-trash"></i>
                  <span>Remover Prédio</span>
                </button>
              )}
            </div>
          )}
        ...
        <div className="flex items-center justify-between pt-2">
          <button
            type="submit"
            className={`${
              selectedPredioId ? "bg-blue-600 hover:bg-blue-700" : "bg-emerald-600 hover:bg-emerald-700"
            } text-white px-5 py-2.5 rounded-lg text-xs font-bold transition-colors cursor-pointer shadow-sm flex items-center space-x-2`}
          >
            <i className={`fa-solid ${selectedPredioId ? "fa-floppy-disk" : "fa-plus"}`}></i>
            <span>{selectedPredioId ? "Gravar Alterações do Prédio" : "Cadastrar Edifício"}</span>
          </button>
          ...
          {selectedPredioId && loggedUser.role === "ADMIN" && (
            <button
              type="button"
              onClick={() => handleRemoverPredio(selectedPredioId)}
              className="text-red-600 hover:text-red-800 text-xs font-bold cursor-pointer underline hover:no-underline"
            >
              Remover este prédio do sistema
            </button>
          )}
        </div>
      </form>
