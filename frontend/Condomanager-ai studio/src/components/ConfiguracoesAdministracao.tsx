...
import { downloadBlob, exportToXLS } from "../utils";
import { 
...
export function ConfiguracoesAdministracao({
  ...
  const [notifCanalSMS, setNotifCanalSMS] = useState(false);
  ...
  const handleExportManualDocument = (format: "pdf" | "doc") => {
    ...
    if (format === "doc") {
      ...
      try {
        ...
        pdf.setFontSize(13);
        pdf.text("CONDOMANAGER AI - MANUAL TÉCNICO OFICIAL", 14, 15);
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
          ...
          <form onSubmit={handleSaveGerais} className="space-y-4 max-w-2xl">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-850">
              <button
                type="submit"
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-5 rounded-lg text-xs cursor-pointer shadow flex items-center gap-1.5"
              >
                <CheckCircle className="h-4 w-4" />
                <span>Guardar Alterações do Edifício</span>
              </button>
            </div>
          </form>
        </div>
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="bg-purple-600 hover:bg-purple-700 text-white text-[11px] font-bold px-4 py-1.5 rounded-lg shadow transition-colors cursor-pointer"
                        >
                          Gravar Modelo
                        </button>
                      </div>
            ...
            <form onSubmit={handleSaveEmailIA} className="space-y-4 max-w-3xl animate-fadeIn">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 max-w-md">
                  ...
                  <button
                    type="submit"
                    className="bg-purple-600 hover:bg-purple-700 text-white font-bold px-4 rounded-lg text-xs cursor-pointer shadow"
                  >
                    Gravar
                  </button>
                </div>
          ...
          <div className="space-y-4 max-w-2xl">
            ...
            <div className="pt-2">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-1.5 px-4 rounded text-xs cursor-pointer shadow"
              >
                Gravar Configurações de Notificação
              </button>
            </div>
          </div>
        </div>
