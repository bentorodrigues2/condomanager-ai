...
import { computeTransferCode, copyTextToClipboard, exportToXLS } from "../utils";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm no-print">
        ...
        <div className="flex space-x-2">
          <button onClick={exportarCondonimosXLS} className="bg-emerald-50 border border-emerald-200 text-emerald-700 font-semibold px-3 py-1.5 rounded-lg text-xs hover:bg-emerald-100 transition-all cursor-pointer">
            <i className="fa-solid fa-file-excel mr-1"></i> Exportar XLS Condóminos
          </button>
          <button onClick={exportarCondonimosPDF} className="bg-slate-100 border border-slate-200 text-slate-700 font-semibold px-3 py-1.5 rounded-lg text-xs hover:bg-slate-200 transition-all cursor-pointer">
            <i className="fa-solid fa-print mr-1"></i> Imprimir Fichas PDF
          </button>
        </div>
      </div>
      ...
      {currentSubTab === "fracoes_proprietario" && (loggedUser.role === 'ADMIN' || loggedUser.role === 'EMPRESA_GESTORA') && (
        <form onSubmit={submeterForm} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6 no-print">
          ...
          <button type="submit" className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-xs font-semibold hover:bg-emerald-700 transition-colors cursor-pointer flex items-center space-x-2">
            ...
            <span>Gravar Ficha do Proprietário</span>
          </button>
        </form>
      )}
      ...
      {(() => {
        ...
        return (
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4 animate-fadeIn">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center space-x-2.5">
                <span className="p-2 bg-slate-900 text-white rounded-lg flex items-center justify-center"><i className="fa-solid fa-folder-open"></i></span>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              ...
              <div className="bg-slate-50/60 p-4 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between border-b border-slate-200/50 pb-2">
                  <span className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center"><i className="fa-solid fa-user-tie text-emerald-600 mr-1.5"></i> Ficha do Proprietário</span>
                  ...
                  <div className="flex flex-col space-y-3">
                    <div className="flex items-center space-x-3">
                      {selectedFracao.proprietario.foto ? (
                        ...
                        <div className="h-12 w-12 rounded-full bg-slate-200 text-slate-500 flex items-center justify-center text-lg border border-slate-300 font-semibold uppercase">{selectedFracao.proprietario.nome.slice(0, 2)}</div>
                      )}
                      <div>
              ...
              {isEditingPermilages ? (
                <>
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold px-3 py-1 rounded transition-colors cursor-pointer"
                  >
                    ✓ Gravar Permilagens
                  </button>
                  <button
